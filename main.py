...
def validar_variables() -> bool:
    ...
    try:
        ...
        # Validar que los IDs de tema son números positivos
        for tema_id, nombre in [
            (TEMA_BOTON_CONSULTAS_COMITE, "TEMA_BOTON_CONSULTAS_COMITE"),
            (TEMA_BOTON_SUGERENCIAS_COMITE, "TEMA_BOTON_SUGERENCIAS_COMITE"),
            (TEMA_CONSULTAS_EXTERNO, "TEMA_CONSULTAS_EXTERNO"),
            (TEMA_SUGERENCIAS_EXTERNO, "TEMA_SUGERENCIAS_EXTERNO"),
            (TEMA_DOCUMENTACION, "TEMA_DOCUMENTACION")
        ]:
            if tema_id <= 0:
                logger.warning(f"{nombre} ({tema_id}) es inválido (debe ser positivo).")
...
async def post_initial_buttons(context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    # Botón de Consultas
    msg_con = (
        "Si no has encontrado la información que buscas en las secciones del grupo "
        "(permisos, bolsa de horas, excedencias, etc...), pulsa el siguiente botón y envíanos un mensaje.\n"
    ...
    # Usar deep_link con el comando start y un payload
    kb_con = [[InlineKeyboardButton("Iniciar Consulta 🙋‍♂️", url=f"https://t.me/{bot_username}?start=iniciar_consulta")]]
    ...
    # Usar deep_link con el comando start y un payload
    kb_sug = [[InlineKeyboardButton("Iniciar Sugerencia 💡", url=f"https://t.me/{bot_username}?start=iniciar_sugerencia")]]
...
async def documentacion_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not chat or chat.type != 'private':
        logger.warning(f"Intento de usar /documentacion fuera de chat privado por {user.id if user else '?'}")
        return
    
    # Actualizar URLs para usar el ID correcto del grupo
    group_id_short = get_short_committee_id()
    keyboard = [
        [InlineKeyboardButton("Calendario Laboral", url=f"https://t.me/c/{group_id_short}/11")],
        [InlineKeyboardButton("Tablas Salariales 2025", url=f"https://t.me/c/{group_id_short}/12")],
        [InlineKeyboardButton("Convenio", url=f"https://t.me/c/{group_id_short}/13")],
        [InlineKeyboardButton("Protocolo Acoso", url=f"https://t.me/c/{group_id_short}/14")]
    ]
    ...
    try:
        ...
        await update.message.reply_text("✅ Botones de documentación enviados con éxito al grupo.")
    except Exception as e:
        logger.error(f"Error enviando Documentación a T:{TEMA_DOCUMENTACION}: {e}")
        await update.message.reply_text("❌ Error al enviar los botones de documentación.")
...
async def start_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> Optional[int]:
    ...
    if not chat or chat.type != 'private':
        logger.info(f"/start ignorado en chat no privado ({chat.id if chat else '?'}, tipo: {chat.type if chat else '?'})")
        return None
    
    args = context.args
    logger.info(f"/start de {user.id if user else '?'} en chat:{chat.id} - Args: {args}")
    ...
    if args:
        ...
        if action_type:
            ...
            if action_type == "consulta":
                prompt = (
                    ...
                    "puedes enviarnos un mensaje. Recuerda que estas consultas son privadas y solo pueden verlas los miembros del comité. "
                    "La consulta debe ser enviada en un solo mensaje."
                )
            else:  # sugerencia
                prompt = (
                    "Si tienes alguna sugerencia para mejorar el grupo o el funcionamiento del comité, "
                    "puedes enviarnos un mensaje. Recuerda que estas sugerencias son privadas y solo pueden verlas los miembros del comité. "
                    "La sugerencia debe ser enviada en un solo mensaje."
                )
        ...
        await update.message.reply_text(
            "Hola, para enviar una consulta o sugerencia, por favor, usa los botones en el grupo del Comité:",
            reply_markup=reply_markup
        )
        raise ApplicationHandlerStop()
        return ConversationHandler.END
        ...
        try:
            ...
            try:
                ...
                keyboard = [[InlineKeyboardButton(
                    f"Nueva {action_type}", 
                    url=f"https://t.me/{bot_username}?start=iniciar_{action_type}"
                )]]
                reply_markup = InlineKeyboardMarkup(keyboard)
                
                await update.message.reply_text(
                    f"✅ ¡Tu {action_type} ha sido enviada correctamente! Gracias.\n\n"
                    f"Si deseas enviar otra {action_type}, puedes usar el botón de abajo:",
                    reply_markup=reply_markup
                )
...
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        await update.message.reply_text(
            "Operación cancelada. Puedes empezar de nuevo usando los botones:", 
            reply_markup=reply_markup
        )
    except Exception as e:
...
async def handle_unexpected_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not chat or chat.type != 'private' or not update.message:
        return
    ...
    bot_username = context.bot.username
    keyboard = [
        [InlineKeyboardButton("Iniciar Consulta", url=f"https://t.me/{bot_username}?start=iniciar_consulta")],
        [InlineKeyboardButton("Iniciar Sugerencia", url=f"https://t.me/{bot_username}?start=iniciar_sugerencia")]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    
    mensaje = (
        "⚠️ Ya has enviado tu consulta o sugerencia o no has iniciado el proceso adecuadamente.\n\n"
        "Si deseas enviar una consulta o sugerencia, por favor, pulsa uno de los botones para iniciar el proceso:"
    )
...
def main() -> None:
    ...
    # Añadir los manejadores con grupos de prioridad
    application.add_handler(conv_handler, group=0)
    application.add_handler(CommandHandler("postbotones", post_buttons_command), group=1)
    application.add_handler(CommandHandler("documentacion", documentacion_command), group=1)
