...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int | None:
    ...
    # Registrar el estado previo si había alguno
    prev_state = "Ninguno" if not context.user_data else f"Tenía: {context.user_data.get('action_type', 'estado desconocido')}"
    logger.info(f"Estado previo de user {user.id}: {prev_state}")
    ...
    if chat and chat.type == "private" and args:
        ...
        if action_type: 
            context.user_data.clear()  # Limpiar cualquier conversación previa
            logger.info(f"Payload '{payload}' de {user.id}. Iniciando {action_type}.")
            context.user_data['action_type'] = action_type
            prompt = f"¡Hola {user.first_name}! Escribe tu {action_type}."
            await update.message.reply_text(prompt)
            return TYPING_REPLY
        else: 
            logger.warning(f"Payload desconocido '{payload}' de {user.id}.")
        ...
        await update.message.reply_text(f"¡Hola {user.first_name}! Usa los botones del grupo.")
        context.user_data.clear()
        return ConversationHandler.END
    return None
...
async def handle_unexpected_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Ya no necesitamos verificar action_type aquí porque:
    # 1. Si estamos en una conversación activa, este handler no se activará
    # 2. Si la conversación terminó, action_type ya se eliminó con pop()
    
    logger.info(f"Mensaje inesperado de {user.id}: {update.message.text[:50]}...")
    if not GRUPO_ID or GRUPO_ID >= 0: 
        await update.message.reply_text("Usa los botones del grupo para hacer una consulta o sugerencia.")
        return
        
    try: 
        short_id = str(GRUPO_ID).replace("-100", "", 1)
        url_con = f"https://t.me/c/{short_id}/{TEMA_BOTON_CONSULTAS_COMITE}"
        url_sug = f"https://t.me/c/{short_id}/{TEMA_BOTON_SUGERENCIAS_COMITE}"
        texto = "Para enviar consultas o sugerencias, por favor usa los botones en los temas del grupo:"
        kb = [
            [InlineKeyboardButton("Ir a Consultas 🤔", url=url_con)], 
            [InlineKeyboardButton("Ir a Sugerencias ✨", url=url_sug)]
        ]
        markup = InlineKeyboardMarkup(kb)
        await update.message.reply_text(texto, reply_markup=markup)
    except Exception as e: 
        logger.error(f"Error handle_unexpected_message {user.id}: {e}", exc_info=True)
        await update.message.reply_text("Por favor, usa los botones del grupo para enviar consultas o sugerencias.")
